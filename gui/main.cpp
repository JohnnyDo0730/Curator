...
static const wchar_t* kRoleNames[] = {
...
static const int kRoleCount = 17;
...
void RefreshRoleList(HWND hwnd) {
    ...
    for (int i = 0; i < kRoleCount; ++i) {
        ...
        lvi.iItem = i;
        lvi.iSubItem = 0;
        lvi.pszText = (LPWSTR)role.c_str();
        SendMessageW(hLV, LVM_INSERTITEMW, 0, (LPARAM)&lvi);
        
        ListView_SetItemText(hLV, i, 1, (LPWSTR)(fileVal.empty() ? L"(預設)" : fileVal.c_str()));
    }
...
void UpdateRoleEditor(HWND hwnd) {
    ...
    if (!hasSel) {
        ...
        std::wstring role = kRoleNames[g_sel_role];
        SetWindowTextW(GetDlgItem(hwnd, IDC_LABEL_ROLE), (L"選中: " + role).c_str());
...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE, LPWSTR, int nCmdShow) {
    ...
    RECT rc = { 0, 0, Scale(430), Scale(520) };
...
void CreateTab0Controls(HWND hwnd) {
    ...
    // 套裝資料夾
    CreateWindowExW(0, L"STATIC", L"套裝資料夾:", WS_CHILD | WS_VISIBLE | SS_LEFT,
    ...
    CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", g_config.packages_path.c_str(),
        ...
        Scale(25), Scale(y), Scale(295), Scale(25), hwnd, (HMENU)IDC_EDIT_PATH, hInst, NULL);
    CreateWindowExW(0, L"BUTTON", L"選擇", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        Scale(330), Scale(y), Scale(70), Scale(25), hwnd, (HMENU)IDC_BTN_BROWSE, hInst, NULL);
    y += 40;

    // 輪循執行 GroupBox
    CreateWindowExW(0, L"BUTTON", L"輪循執行", WS_CHILD | WS_VISIBLE | BS_GROUPBOX,
        Scale(25), Scale(y), Scale(375), Scale(155), hwnd, (HMENU)IDC_GROUP_LOOP, hInst, NULL);
    ...
    CreateWindowExW(0, L"BUTTON", L"啟動", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        Scale(100), Scale(gy), Scale(85), Scale(27), hwnd, (HMENU)IDC_BTN_START, hInst, NULL);
    CreateWindowExW(0, L"BUTTON", L"終止", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        Scale(210), Scale(gy), Scale(85), Scale(27), hwnd, (HMENU)IDC_BTN_STOP, hInst, NULL);

    y += 165;

    // 單次執行 GroupBox
    CreateWindowExW(0, L"BUTTON", L"單次執行", WS_CHILD | WS_VISIBLE | BS_GROUPBOX,
        Scale(25), Scale(y), Scale(375), Scale(148), hwnd, (HMENU)IDC_GROUP_SINGLE, hInst, NULL);
    ...
    HWND hComboPack = CreateWindowExW(0, L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST,
        Scale(218), Scale(gy), Scale(142), Scale(160), hwnd, (HMENU)IDC_COMBO_PACK, hInst, NULL);
    // 填入套裝
    for (auto &pkg : g_config.packages)
    ...
    CreateWindowExW(0, L"BUTTON", L"執行", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        Scale(100), Scale(gy), Scale(85), Scale(27), hwnd, (HMENU)IDC_BTN_EXECUTE_SINGLE, hInst, NULL);
    CreateWindowExW(0, L"BUTTON", L"恢復預設", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        Scale(210), Scale(gy), Scale(85), Scale(27), hwnd, (HMENU)IDC_BTN_RESTORE_DEFAULT, hInst, NULL);
}
...
void CreateTab1Controls(HWND hwnd) {
    ...
    // 提示與選擇區域
    CreateWindowExW(0, L"STATIC", L"套裝根資料夾:", WS_CHILD | SS_LEFT,
        Scale(25), Scale(y), Scale(100), Scale(20), hwnd, (HMENU)IDC_LABEL_PKG_PATH, hInst, NULL);
    CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", g_config.packages_path.c_str(),
        WS_CHILD | WS_BORDER | ES_AUTOHSCROLL | ES_READONLY,
        Scale(125), Scale(y-2), Scale(205), Scale(24), hwnd, (HMENU)IDC_EDIT_PKG_PATH, hInst, NULL);
    CreateWindowExW(0, L"BUTTON", L"...", WS_CHILD | BS_PUSHBUTTON,
        Scale(335), Scale(y-2), Scale(30), Scale(24), hwnd, (HMENU)IDC_BTN_PKG_BROWSE, hInst, NULL);
    y += 35;

    CreateWindowExW(0, L"STATIC", L"選擇要編輯的套裝:", WS_CHILD | SS_LEFT,
        Scale(25), Scale(y), Scale(120), Scale(20), hwnd, (HMENU)IDC_LABEL_PKG_HINT, hInst, NULL);
    CreateWindowExW(0, L"COMBOBOX", L"", WS_CHILD | CBS_DROPDOWNLIST,
        Scale(150), Scale(y-2), Scale(180), Scale(200), hwnd, (HMENU)IDC_COMBO_PKG_SELECT, hInst, NULL);
    
    CreateWindowExW(0, L"BUTTON", L"儲存所有設定", WS_CHILD | BS_PUSHBUTTON,
        Scale(335), Scale(y-2), Scale(85), Scale(26), hwnd, (HMENU)IDC_BTN_PKG_SAVE, hInst, NULL);
    y += 40;

    // 腳色映射區塊 (佔據大部分空間)
    CreateWindowExW(0, L"BUTTON", L"角色映射設置 (選中後點擊 [...] 變更或直接輸入)", WS_CHILD | BS_GROUPBOX,
        Scale(25), Scale(y), Scale(395), Scale(360), hwnd, (HMENU)IDC_GROUP_PKG_MAP, hInst, NULL);

    int gy = y + 30;
    // 腳色映射區塊 (ListView)
    HWND hLV = CreateWindowExW(WS_EX_CLIENTEDGE, WC_LISTVIEWW, L"",
        WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_SINGLESEL | LVS_SHOWSELALWAYS,
        Scale(40), Scale(gy), Scale(365), Scale(270), hwnd, (HMENU)IDC_LIST_ROLES, hInst, NULL);
    
    // 設定 ListView 欄位 (Reg名稱 與 檔名)
    LVCOLUMNW lvc = { 0 };
    lvc.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM;
    lvc.pszText = (LPWSTR)L"Reg角色";
    lvc.cx = Scale(110);
    SendMessageW(hLV, LVM_INSERTCOLUMNW, 0, (LPARAM)&lvc);
    
    lvc.pszText = (LPWSTR)L"檔案名稱";
    lvc.cx = Scale(230);
    ...
    gy += 280;
    // 快速編輯/瀏覽區
    CreateWindowExW(0, L"STATIC", L"角色: 請選擇", WS_CHILD | SS_LEFT,
        Scale(40), Scale(gy+4), Scale(120), Scale(20), hwnd, (HMENU)IDC_LABEL_ROLE, hInst, NULL);
    
    CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
        WS_CHILD | WS_BORDER | ES_AUTOHSCROLL,
        Scale(165), Scale(gy), Scale(195), Scale(25), hwnd, (HMENU)IDC_EDIT_FILE, hInst, NULL);

    CreateWindowExW(0, L"BUTTON", L"...", WS_CHILD | BS_PUSHBUTTON,
        Scale(365), Scale(gy), Scale(35), Scale(25), hwnd, (HMENU)IDC_BTN_FILE_BROWSE, hInst, NULL);
...
static const int kTab1IDs[] = {
  ...
  IDC_EDIT_FILE, IDC_BTN_FILE_BROWSE
};
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        HWND hTab = CreateWindowExW(0, WC_TABCONTROLW, L"",
            ...
            Scale(10), Scale(10), Scale(410), Scale(498),
    ...
    case WM_COMMAND: {
        ...
        case IDC_BTN_PKG_SAVE: {
            saveConfig(g_config, g_config_path);
            MessageBoxW(hwnd, L"套裝與角色映射設定已儲存至 config.json", L"儲存成功", MB_OK | MB_ICONINFORMATION);
            break;
        }
        ...
        case IDC_EDIT_FILE:
            if (wmEvent == EN_CHANGE && !g_is_refreshing_ui) {
                if (g_sel_pkg >= 0 && g_sel_role >= 0) {
                    ...
                    g_config.packages[g_sel_pkg].mapping[kRoleNames[g_sel_role]] = buf;
                    
                    // 僅更新 ListView 的子項目字串，不觸發完整 RefreshRoleList 以避免過度刷新
                    HWND hLV = GetDlgItem(hwnd, IDC_LIST_ROLES);
                    ListView_SetItemText(hLV, g_sel_role, 1, (LPWSTR)(wcslen(buf) > 0 ? buf : L"(預設)"));
                }
            }
            break;
        ...
        case IDC_BTN_FILE_BROWSE: {
            ...
            if (GetOpenFileNameW(&ofn)) {
                ...
                // 若為 alternative 套裝，且檔案不在套裝目錄內，則自動複製
                if (g_config.packages[g_sel_pkg].name == L"00_alternative") {
                    if (std::filesystem::exists(dstDir)) {
                        ...
                        if (src != target) {
                            ...
                            if (ec) {
                                MessageBoxW(hwnd, (L"無法將檔案複製到 alternative 資料夾: " + toW(ec.message())).c_str(), L"錯誤", MB_OK | MB_ICONERROR);
                            }
                        }
                    }
                }
    ...
    case WM_USER_TASK_DONE: {
        ...
        if      (action == 1)  MessageBoxW(hwnd, L"已建立系統排程並立即套用一次游標", L"啟動成功", MB_OK | MB_ICONINFORMATION);
        else if (action == 2)  MessageBoxW(hwnd, L"已刪除背景輪循排程", L"終止成功", MB_OK | MB_ICONINFORMATION);
