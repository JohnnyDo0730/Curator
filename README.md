...
現在您不再需要自行編譯專案，我們已提供打包好的執行檔：

1.  前往本專案的 [**Releases**](https://github.com/JohnnyDo0730/Curator/releases) 頁面。
2.  下載最新的 `Curator_v1.x.zip`。
3.  解壓縮至您喜歡的目錄（例如 `D:\Tools\Curator`）。
4.  將您的鼠標包資料夾放入 `cursors/` 目錄中。
5.  執行 `curator_gui.exe` 開始設定！

---

## 🐞 問題回報與兼容性徵集

由於 Windows 版本與登錄檔權限的多樣性，本工具可能在某些環境下遇到非預期行為。

如果您在使用中遇到：
- 鼠標套用後沒有即刻重新整理 (Refresh)。
- 排程器啟動失敗。
- 介面顯示異常。

請務必透過 [**GitHub Issues**](https://github.com/JohnnyDo0730/Curator/issues) 回報問題。我們特別希望收集不同 Windows 版本（Win10/Win11 各版本號）的兼容性資訊，您的回饋將幫助我們優化核心引擎。
...
如果您是開發者並希望自行編譯，本專案採用模組化設計：`core/` (核心邏輯), `gui/` (圖形介面), `cli/` (背景引擎)。
